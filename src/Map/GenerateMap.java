...
public class GenerateMap {
	...
	private boolean addItems(int[][] map,ArrayList<Vectors> boxes,ArrayList<Vectors> goal,Vectors player){
		...
		//	System.out.println("box "+ i+" "+boxes.get(i).getX()+" "+ boxes.get(i).getY());
	...
	private boolean moveItems(int map[][],ArrayList<Vectors> boxes,ArrayList<Vectors> goal,Vectors player){
		...
		while(move!=0&&test<500){
			...
			System.out.println("be chosen is box " + boxes.get(boxnum).getX()+" "+boxes.get(boxnum).getY()+" "+boxnum);
	...
	private boolean Asearch(Vectors from,Vectors to,int[][] map){
		...
		while(!open.isEmpty()){
			...
			//System.out.println("cur " + cur.getX() +" " + cur.getY()+" value "+cur.getGs()+" "+cur.getFx());
			//close.add(cur);
			if(cur.getX()!=0){
				...
				if (add.getX()==to.getX()&&add.getY()==to.getY()){
				...
				else if((map[cur.getX()-1][cur.getY()]==0||map[cur.getX()-1][cur.getY()]==4)&&!open.contains(add)&&!close.contains(add)){
					...
					if(!contains){
					//System.out.println("add is " + add.getX()+" " +add.getY());
			...
			if(cur.getY()!=0){
				...
				else if((map[cur.getX()][cur.getY()-1]==0||map[cur.getX()][cur.getY()-1]==4)&&!open.contains(add)&&!close.contains(add)){
					boolean contains = false;
					for(int i=0;i<close.size();i++){
						if(close.get(i).getX()==add.getX()&&close.get(i).getY()==add.getY()){
							System.out.println("contained");
	...
	private boolean badplace(Vectors player,int [][]map){
		if(player.getX()>0&&player.getX()<map.length-1&&player.getY()>0&&player.getY()<map.length-1){
			if(map[player.getX()-1][player.getY()]!=0&&map[player.getX()+1][player.getY()]!=0&&map[player.getX()][player.getY()-1]!=0&&map[player.getX()][player.getY()+1]!=0){
				return true;
			}
		}else if(player.getX()==0&&player.getX()<map.length-1&&player.getY()>0&&player.getY()<map.length-1){
			if(map[player.getX()+1][player.getY()]!=0&&map[player.getX()][player.getY()-1]!=0&&map[player.getX()][player.getY()+1]!=0){
				return true;
			}
		}else if(player.getX()==map.length-1&&player.getX()>0&&player.getY()>0&&player.getY()<map.length-1){
			if(map[player.getX()-1][player.getY()]!=0&&map[player.getX()][player.getY()-1]!=0&&map[player.getX()][player.getY()+1]!=0){
				return true;
			}
		}else if(player.getY()==map.length-1&&player.getX()>0&&player.getX()<map.length-1&&player.getY()>0){
			if(map[player.getX()-1][player.getY()]!=0&&map[player.getX()+1][player.getY()]!=0&&map[player.getX()][player.getY()-1]!=0){
				return true;
			}
		}else if(player.getY()==0&&player.getY()<map.length-1&&player.getX()>0&&player.getX()<map.length-1){
			if(map[player.getX()-1][player.getY()]!=0&&map[player.getX()+1][player.getY()]!=0&&map[player.getX()][player.getY()+1]!=0){
				return true;
			}
		}
